...
def detect_traces(
    ...
    ntraces, trace_pts = _link_peaks_to_traces_mtt(pk_grid, nsteps, max_ntraces, MAXD)
...
def _link_peaks_to_traces_mtt(
    ...
    *,
    ...
    # ---- Step 0: find first non-empty sequence and initialize tracks ----
    start_seq = -1
    for s in range(nsteps):
        ...
        if peaks.size:
            ...
            trace_pts[:n_init, s] = peaks[:n_init]
            last_step[:n_init] = s
    ...
    # ---- Main MTT loop over sequences ----
    for s in range(start_seq + 1, nsteps):
        peaks = pk_grid[s]
        peaks = peaks[peaks > 0.0]
        if peaks.size == 0:
            continue
        ...
        gaps = (s - last_step[:ntracks]).astype(np.int32)
        ...
        # This is the big O(m*n) step; then we shrink the LAP to only viable rows/cols.
        ...
        # These correspond to N_POTENT_ASSOC_T2P and N_POTENT_ASSOC_P2T in Fortran.
        ...
        # Points with no viable track: they become new tracks immediately (like filtered out)
        ...
        gap_sub = (s - last_step[lap_trk]).astype(np.int32)  # shape (m,)
        # viable_sub is |dx| <= max_displacement (gap already limited)
        viable_sub = abs_dx_sub <= max_displacement

        m = lap_trk.size
        n = lap_peaks.size

        # ---- Form LAP cost matrix with dummies for "missing" assignments ----
        # We solve track->point assignment:
        #   rows: tracks (m)
        #   cols: points (n) + dummy_missing (m)
        # Each track picks at most one real point, else dummy => missing this step.
        dtype_cost = np.float32 if use_float32 else np.float64
        cost = np.full((m, n + m), missing_cost, dtype=dtype_cost)

        # Fill valid real costs using Euclidean in (x, seq) space:
        # cost = sqrt(dx^2 + gap^2)
        # gap is per-row, so broadcast.
        if np.any(viable_sub):
            # Use float computation with minimal temporaries
            gap_f = gap_sub.astype(dtype_cost, copy=False)[:, None]
            dx_f = dx_sub.astype(dtype_cost, copy=False)
            # Only compute where viable to save time
            # (Still creates arrays, but avoids sqrt on invalid pairs.)
        ...
        for r, c in zip(row_ind, col_ind):
            if r >= m:
                continue
            if c < n:
                # Real point assignment; ensure it is a gated match (otherwise treat as missing)
